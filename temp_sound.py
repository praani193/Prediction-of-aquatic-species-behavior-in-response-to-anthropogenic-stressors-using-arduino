...
def show_graphs():
    ...
    line1, = ax1.plot(x, temperature, label='Temperature', color='blue')
    ax1.set_xlabel('Time')
    ...
    line2, = ax2.plot(x, frequency, label='Frequency', color='red')
    ax2.set_xlabel('Time')
    ax2.set_ylabel('Frequency')
    ax2.set_title('Frequency vs. Time')
    ax2.legend()

    canvas1 = FigureCanvasTkAgg(fig1, master=graph_win)
    canvas1.get_tk_widget().pack(side=LEFT, fill=BOTH, expand=1)
    canvas2 = FigureCanvasTkAgg(fig2, master=graph_win)
    ...
    ani = animation.FuncAnimation(fig1, animate, interval=1000)  # Update every 1 second
...
def check1():
    ...
    if data == pass_word:
        ...
        b1 = Button(filemenu, text='Run', width=20, height=1, padx=5, pady=5, bg='black', command=a, activebackground='white',
                    font=('Helvetica', 25, 'bold'), fg='purple', bd=0)
        b2 = Button(filemenu, text='Rs from\n Temperature sensor', width=20, height=2, padx=5, pady=5, bg='black',
                    command=b, activebackground='white', font=('Helvetica', 25, 'bold'), fg='purple', bd=0)
        b3 = Button(filemenu, text='Rs from\n Sound sensor', width=20, height=2, padx=5, pady=5, bg='black', command=c,
                    activebackground='white', font=('Helvetica', 25, 'bold'), fg='purple', bd=0)
